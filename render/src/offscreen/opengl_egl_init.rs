...
pub fn opengl_initialize_offscreen_rendering(
    ...
    unsafe {
        ...
        if !egl::bind_api(egl::EGL_OPENGL_API) {
            Err(RenderInitError::ApiNotAvailable)?
        }

        let egl_display =
            ffi::eglGetPlatformDisplay(egl::EGL_PLATFORM_GBM_MESA, gbm as *mut c_void, ptr::null());
        let egl_display = if egl_display.is_null() {
            None
        } else {
            Some(egl_display)
        };
        let egl_display = if let Some(egl_display) = egl_display {
            egl_display
        } else {
            println!("eglGetPlatformDisplay {:x}", egl::get_error());
        ...
        if !init_result {
            println!("egl::initialize {:x}", egl::get_error());
            Err(RenderInitError::CannotStartGraphicsDriver)?
        }
        ...
        let extensions = if let Some(extensions) = extensions {
            ...
            Err(RenderInitError::MissingRequiredExtension)?
        };
        ...
        let config = if let Some(config) = config {
            ...
            println!("egl::choose_config {:x}", egl::get_error());
        ...
        if !activated_context {
            println!("egl::make_current {:x}", egl::get_error());
...
impl OffscreenRenderContext for EglOffscreenRenderContext {
    ...
    fn create_render_target(&mut self, width: usize, height: usize) -> Self::RenderTarget {
        ...
        if !activated_context {
            panic!("egl::make_current {:x}", egl::get_error());
        }
