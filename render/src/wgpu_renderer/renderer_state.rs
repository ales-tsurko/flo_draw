...
pub(crate) struct RendererState {
    ...
    pub render_pass:
        Vec<Box<dyn for<'a> FnOnce(&'a RenderPassResources, &mut wgpu::RenderPass<'a>) -> ()>>,
...
impl RendererState {
            ...
            self.render_pass
                .push(Box::new(move |resources, render_pass| {
                    render_pass.set_bind_group(
                        matrix_group,
                        &resources.matrix_bind_groups[matrix_buffer_index],
                        &[],
                    );
                }));
        }
    ...
    pub fn bind_current_clip_mask(&mut self) {
        if let Some(pipeline) = &self.pipeline {
            ...
            let clip_binding = pipeline.bind_clip_mask(
                &*self.device,
                clip_texture.as_ref().map(|clip_texture| &**clip_texture),
            );
    ...
    pub fn bind_current_texture(&mut self) {
        if let Some(pipeline) = &self.pipeline {
            ...
            self.render_pass_resources.texture_settings.push((
                pipeline.clone(),
                texture_settings,
                input_texture,
                sampler,
            ...
            self.render_pass
                .push(Box::new(move |resources, render_pass| {
                    render_pass.set_bind_group(
                        texture_group,
                        &resources.texture_settings_bind_groups[settings_buffer_index],
                        &[],
                    );
                }));
        }
