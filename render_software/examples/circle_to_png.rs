...
pub fn main() {
    ...
    let circle = ContourEdge::new(
        (960.0 - radius, 540.0 - radius),
        circle_shape,
        CircularDistanceField::with_radius(radius),
    );
    let background = RectangleEdge::new(background_shape, 0.0..1920.0, 0.0..1080.0);

    let background_color = program_cache.store_program_data(
        &solid_color,
        &mut data_cache,
        SolidColorData(F32LinearPixel::from_components([0.0, 0.0, 0.0, 0.0])),
    );
    let foreground_color = program_cache.store_program_data(
        &solid_color,
        &mut data_cache,
        SolidColorData(F32LinearPixel::from_components([0.0, 0.0, 1.0, 1.0])),
    );
    ...
    let edge_plan = EdgePlan::<Box<dyn EdgeDescriptor>>::new()
        .with_shape_description(
            background_shape,
            ShapeDescriptor {
                programs: smallvec![background_color],
                is_opaque: true,
                z_index: 0,
            },
        )
        .with_shape_description(
            circle_shape,
            ShapeDescriptor {
                programs: smallvec![foreground_color],
                is_opaque: true,
                z_index: 1,
            },
        )
