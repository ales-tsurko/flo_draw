...
type SimpleSpriteProgram<TPixel> =
    BasicSpriteProgram<TPixel, Arc<dyn EdgeDescriptor>, PixelScanPlanner<Arc<dyn EdgeDescriptor>>>;
type AffineSpriteProgram<TPixel> = TransformedSpriteProgram<
    TPixel,
    Arc<dyn EdgeDescriptor>,
    PixelScanPlanner<Arc<dyn EdgeDescriptor>>,
>;
    ...
    TPixel: 'static + Send + Sync + Pixel<N>,
{
    ...
    pub(super) basic_texture:
        StoredPixelProgramFromProgram<BasicTextureProgram<TPixel, RgbaTexture>>,
    ...
    TPixel: 'static + Send + Sync + Pixel<N>,
{
    fn default() -> Self {
        ...
        let basic_sprite =
            cache.add_pixel_program::<SimpleSpriteProgram<TPixel>>(BasicSpriteProgram::default());
        let transformed_sprite = cache.add_frame_pixel_program::<AffineSpriteProgram<TPixel>>(
            TransformedSpriteProgram::default(),
        );
