    ...
    TProgramRunner::TPixel: 'static + Send + Copy + AlphaBlend,
{
    ...
    fn render(&self, region: &Self::Region, source: &Self::Source, dest: &mut Self::Dest) {
        ...
        if (scanline.len() as f64) < end_pos.floor() {
            panic!(
                "Scanline is too long (have {} pixels, but want to write {})",
                end_pos,
                scanline.len()
            );
        }
        ...
        if start_pos < 0.0 {
            panic!(
                "Scanline starts before the start of the list of pixels (at {})",
                start_pos
            );
        }
            ...
            loop {
                ...
                match current_step {
                    PixelProgramPlan::Run(data_id) => {
                        ...
                        self.program_data.run_program(
                            *data_id,
                            shadow_pixels.buffer(),
                            pixel_range,
                            transform,
                            y_pos,
                        );
                    ...
                    PixelProgramPlan::LinearSourceOver(start, end) => {
                        ...
                        let multiplier = if x_len > 1 {
                            (end - start) / ((x_len - 1) as f64)
                        } else {
                            0.0
                        };
...
mod test {
    ...
    pub fn linear_source_over_start_at_zero() {
        ...
        let white_pixel = BasicPixelProgramRunner::from(
            |_, data: &mut [F32LinearPixel], x_range: Range<i32>, _: &_, _| {
                data[(x_range.start as usize)..(x_range.end as usize)]
                    .iter_mut()
                    .for_each(|pixel| *pixel = F32LinearPixel::white());
            },
        );
        ...
        let plan = ScanlinePlan::from_ordered_stacks(vec![ScanSpanStack::with_reversed_programs(
            0.0..10.0,
            false,
            &vec![
                PixelProgramPlan::LinearSourceOver(0.0, 1.0),
                PixelProgramPlan::Run(PixelProgramDataId(0)),
                PixelProgramPlan::StartBlend,
            ],
        )]);
        let region = ScanlineRenderRegion {
            y_pos: 0.0,
            transform: ScanlineTransform::identity(),
        };
    ...
    pub fn linear_source_over_start_at_half() {
        ...
        let plan = ScanlinePlan::from_ordered_stacks(vec![ScanSpanStack::with_reversed_programs(
            ...
            false,
            &vec![
                PixelProgramPlan::LinearSourceOver(0.5, 1.0),
                PixelProgramPlan::Run(PixelProgramDataId(0)),
                PixelProgramPlan::StartBlend,
            ],
        )]);
    ...
    pub fn linear_source_over_ends_at_one() {
        ...
        assert!(
            target[9].alpha_component() == 1.0,
            "{}",
            target[9].alpha_component()
        );
    }
    ...
    pub fn linear_source_over_ends_at_half() {
        ...
        let plan = ScanlinePlan::from_ordered_stacks(vec![ScanSpanStack::with_reversed_programs(
            ...
            false,
            &vec![
                PixelProgramPlan::LinearSourceOver(0.0, 0.5),
                PixelProgramPlan::Run(PixelProgramDataId(0)),
                PixelProgramPlan::StartBlend,
            ],
        )]);
    ...
    pub fn linear_source_over_blending() {
        ...
        // The start alpha is 0.5 so that should be the value written to the initial pixel
        assert!(
            target[0].alpha_component() == 1.0,
            "{}",
            target[0].alpha_component()
        );
