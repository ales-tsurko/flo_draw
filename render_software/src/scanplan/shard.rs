...
fn resolve_shards(
    previous_line: &SmallVec<[EdgeDescriptorIntercept; 2]>,
    next_line: &SmallVec<[EdgeDescriptorIntercept; 2]>,
) -> SmallVec<[ShardIntercept; 2]> {
    // Mix the previous and next lines and then sort them by edge position
    let mut sorted_lines = previous_line
        .iter()
        .map(|intercept| (intercept, false))
    ...
    {
        let (second_intercept, second_is_next) =
            if first_intercept.position.0 != second.0.position.0 {
    ...
    TInterceptIterator: Iterator<Item = SmallVec<[EdgeDescriptorIntercept; 2]>>,
{
    ...
    fn next(&mut self) -> Option<SmallVec<[ShardIntercept; 2]>> {
        ...
        let mut next_line = if let Some(next_line) = self.intercepts.next() {
            next_line
        } else {
            return None;
        };
...
///
pub fn shard_intercepts_from_edge<'a, TEdge: EdgeDescriptor>(
    edge: &'a TEdge,
    y_positions: &'a [f64],
) -> impl 'a + Iterator<Item = SmallVec<[ShardIntercept; 2]>> {
    ...
    let mut first_line = intercepts
        .next()
        .expect("Must be at least one y-position to generate a shard iterator");
