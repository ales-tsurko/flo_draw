    ...
    fn next(&mut self) -> Option<ShardIntercept> {
        ...
        let next_shard = if let Some(next_shard) = self.next_shard.take() {
            ...
            let result = remaining
                .next()
                .map(|intercept| ShardInterceptLocation::from(intercept, self.transform));
...
///
#[inline]
fn actual_fade_for_range(
    render_x_range: &Range<f64>,
    alpha_x_range: &Range<f64>,
    ...
    if (alpha_x_range.end - alpha_x_range.start).abs() < 1e-6 {
        ...
        let corrected_start_2 =
            ((pixel_x_start + 1.0) - alpha_x_start) * alpha_ratio + alpha_range.start;
        let corrected_end_1 =
            ((pixel_x_end - 1.0) - alpha_x_start) * alpha_ratio + alpha_range.start;
        ...
        let corrected_range =
            (corrected_start.max(0.0).min(1.0))..(corrected_end.max(0.0).min(1.0));
    ...
    TEdge: EdgeDescriptor,
{
    ...
    fn plan_scanlines(
        ...
        let scan_positions = y_positions
            .iter()
            .map(|y| y - half_pixel)
        ...
        'next_line: for y_idx in 0..y_positions.len() {
            ...
            *scanline_pos = y_positions[y_idx];
            ...
            let mut scanline_intercepts =
                ShardInterceptIterator::from_intercepts(scanline_intercepts.into_iter(), transform);
            ...
            let mut current_intercept = if let Some(intercept) = scanline_intercepts.next() {
                intercept
            } else {
                continue;
            };
            ...
            while current_intercept.x_pos() < x_range.start {
                ...
                match &current_intercept {
                    ShardIntercept::Start(intercept) => {
                        active_shapes.start_intercept(intercept, transform, shape_descriptor)
                    }
                    ShardIntercept::Finish(intercept) => {
                        active_shapes.finish_intercept(intercept, shape_descriptor)
                    }
                }

                // Move to the next intercept (or stop if no intercepts actually fall within the x-range)
                current_intercept = if let Some(intercept) = scanline_intercepts.next() {
                    intercept
                } else {
                    continue 'next_line;
                };
            ...
            loop {
                    ...
                    for shape in (0..stack_depth).rev() {
                        ...
                        loop {
                            match blend {
                                ...
                                InterceptBlend::Fade {
                                    x_range: alpha_x_range,
                                    alpha_range,
                                } => {
                                    // Adjust the alpha range to the actual x range
                                    let corrected_range = actual_fade_for_range(
                                        &x_range,
                                        &alpha_x_range,
                                        &alpha_range,
                                    );
                                    ...
                                    program_stack.push(PixelProgramPlan::LinearSourceOver(
                                        corrected_range.start as _,
                                        corrected_range.end as _,
                                    ));
                                    num_blends += 1;
                                    break;
                                }
                        ...
                        if num_blends > 0 {
                            program_stack
                                .extend((0..num_blends).map(|_| PixelProgramPlan::StartBlend));
                        }
                ...
                current_intercept = if let Some(next_intercept) = scanline_intercepts.next() {
                    next_intercept
                } else {
                    break;
                };
