...
fn scan_triangle() {
    ...
    let shards =
        shard_intercepts_from_edge(&triangle, &[99.0, 100.0, 125.0, 150.0, 175.0, 200.0, 201.0])
            .collect::<Vec<_>>();
    ...
    assert!(
        shards[0].len() == 0,
        "99-100 should have no intercepts ({:?})",
        shards
    );
    ...
    assert!(
        shards[1].len() == 2,
        "100-125 should have 2 intercepts ({:?})",
        shards
    );
    assert!(
        (shards[1][0].x_range().start - 100.0).abs() < 0.01,
        "100-125 should start at 100 ({:?})",
        shards
    );
    assert!(
        (shards[1][0].x_range().end - 125.0).abs() < 0.01,
        "100-125 should end at 125 ({:?})",
        shards
    );
    assert!(
        (shards[1][1].x_range().start - 275.0).abs() < 0.01,
        "100-125 should start at 275 ({:?})",
        shards
    );
    assert!(
        (shards[1][1].x_range().end - 300.0).abs() < 0.01,
        "100-125 should end at 300 ({:?})",
        shards
    );
    ...
    assert!(
        shards[2].len() == 2,
        "125-150 should have 2 intercepts ({:?})",
        shards
    );
    assert!(
        shards[3].len() == 2,
        "150-175 should have 2 intercepts ({:?})",
        shards
    );
    assert!(
        shards[4].len() == 0,
        "175-200 should have 0 intercepts ({:?})",
        shards
    );
    assert!(
        shards[5].len() == 0,
        "200-201 should have 0 intercepts ({:?})",
        shards
    );
}
...
fn scan_concave() {
    ...
    let shards = shard_intercepts_from_edge(
        &concave_shape,
        &[99.0, 100.0, 125.0, 150.0, 175.0, 198.0, 199.0, 200.0, 201.0],
    )
    .collect::<Vec<_>>();
