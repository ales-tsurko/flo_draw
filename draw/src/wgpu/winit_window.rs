...
///
pub(super) async fn send_actions_to_window<RenderStream, EventPublisher>(
    window: WinitWindow,
                ...
                WindowUpdate::Render(next_action) => {
                    ...
                    if let (Some(winit_window), None) = (&window.window, &window.renderer) {
                        ...
                        let adapter = instance
                            ...
                            .await
                            .expect("Could not acquire an adapter for winit/wgpu");
                        ...
                        let (device, queue) = adapter
                            ...
                            .await
                            .expect("Create WGPU device and queue");
                        ...
                        let renderer = WgpuRenderer::from_surface(
                            Arc::clone(&device),
                            Arc::clone(&queue),
                            Arc::clone(&surface),
                            Arc::clone(&adapter),
                    ...
                    if let (Some(winit_window), Some(renderer)) =
                        (&window.window, &mut window.renderer)
                    {
                        ...
                        if let Some(next_frame) = maybe_next_frame {
                            ...
                            winit_thread().send_event(WinitThreadEvent::PresentSurface(
                                window_id, next_frame, yield_send,
                            ));
                            ...
                            #[cfg(feature = "profile")]
                            println!(
                                "WINIT: time to present frame {}µs",
