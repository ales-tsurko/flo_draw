...
pub fn main() {
    with_2d_graphics(|| {
        ...
        let mut gpu_instructions =
            pipe(renderer, canvas_stream, |renderer, drawing_instructions| {
                async move {
                    renderer
                        .draw(drawing_instructions.into_iter())
                        .collect::<Vec<_>>()
                        .await
                }
                .boxed()
        ...
        executor::block_on(async {
            ...
            while let Some(gpu_instruction) = gpu_instructions.next().await {
                ...
                match &gpu_instruction {
                    ...
                    RenderAction::DrawIndexedTriangles(
                        vertex_buffer_id,
                        index_buffer_id,
                        num_vertices,
                    ) => {
                        ...
                        tessellation_window.draw(|gc| {
                            ...
                            for triangle_num in 0..(num_vertices / 3) {
                                ...
                                let colour = Color::Rgba(
                                    p1.color[0] as f32 / 255.0,
                                    p1.color[1] as f32 / 255.0,
                                    p1.color[2] as f32 / 255.0,
                                    p1.color[3] as f32 / 255.0,
                                );
